...
function launchParticlesJS(tag_id, params) {
    ...
    pJS.fn.canvasSize = function () {
        ...
        window.addEventListener("resize", pJS.fn.onResize, false);
    };

    pJS.fn.onResize = function(){
        if (pJS) {
    ...
    pJS.fn.particlesAnimate = function () {
        for (var i = 0, len = pJS.particles.array.length; i < len; i++) {
            /* the particle */
            ...
            for (var j = i + 1, len1 = pJS.particles.array.length; j < len1; j++) {
    ...
    pJS.fn.particlesDraw = function () {
        ...
        var i = 0,
            len = pJS.particles.array.length,
            p,
            color;
        ...
        for (; i < len; i++) {
            ...
            color = ['rgba(', p.color.r, ',' , p.color.g , ',' , p.color.b , ',' , p.opacity , ')'].join("");
            p.draw(color);
        }
    ...
    pJS.fn.vendors.distanceParticles = function (p1, p2) {
        ...
        var dx = p1.x - p2.x,
            ...
            line_linked = particles.line_linked;

        /* Check distance between particle and mouse mos */
        if (dist <= particles.line_linked.distance) {

            /* draw the line */
            var color_line = line_linked.color_rgb_line,
                ctx = pJS.canvas.ctx,
                strokeColor = ['rgba(',
                                color_line.r,
                                ",",
                                ...
                                color_line.b,
                                ",",
                                (line_linked.opacity - dist / line_linked.distance),
                                ')'
                            ].join("");
            ctx.beginPath();
            ...
            ctx.lineWidth = line_linked.width;
            ctx.stroke();
            ctx.closePath();

            /* condensed particles */
            if (line_linked.condensed_mode.enable) {
                dx = p1.x - p2.x;
                dy = p1.y - p2.y;
                ax = dx / (line_linked.condensed_mode.rotateX * 1000);
                ay = dy / (line_linked.condensed_mode.rotateY * 1000);
                p2.vx += ax;
                p2.vy += ay;
            }
        }
    ...
    pJS.fn.vendors.interactivity.grabParticles = function (p1, p2) {
        ...
        if (dist <= pJS.particles.line_linked.distance && dist_mouse <= pJS.interactivity.mouse.distance && pJS.interactivity.status == 'mousemove') {
            /* Draw the line */
            ctx.beginPath();
            ctx.strokeStyle = 'rgba(' + color_line.r + ',' + color_line.g + ',' + color_line.b + ',' + (pJS.interactivity.line_linked.opacity - dist_mouse / pJS.interactivity.mouse.distance) + ')';
            ctx.moveTo(p1.x, p1.y);
    ...
    pJS.fn.vendors.destroy = function () {
        ...
        canvas_el.remove();
        window.removeEventListener("resize", pJS.fn.onResize, false);
        pJS = null;
    };

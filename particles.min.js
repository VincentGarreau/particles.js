...
}if(c.interactivity.mode){pJS.interactivity.mode=c.interactivity.mode;}}}pJS.retina_detect=c.retina_detect;}pJS.particles.color_rgb=hexToRgb(pJS.particles.color);
pJS.particles.line_linked.color_rgb_line=hexToRgb(pJS.particles.line_linked.color);if(pJS.retina_detect){if(window.devicePixelRatio>1){pJS.retina=true;
pJS.canvas.w=pJS.canvas.el.offsetWidth*2;pJS.canvas.h=pJS.canvas.el.offsetHeight*2;pJS.particles.anim.speed=pJS.particles.anim.speed*2;pJS.particles.line_linked.distance=pJS.particles.line_linked.distance*2;
pJS.particles.line_linked.width=pJS.particles.line_linked.width*2;pJS.interactivity.mouse.distance=pJS.interactivity.mouse.distance*2;}}pJS.fn.canvasInit=function(){pJS.canvas.ctx=pJS.canvas.el.getContext("2d");
...
}};pJS.fn.vendors.distanceParticles=function(l,j){var g=l.x-j.x,f=l.y-j.y,k=Math.sqrt(g*g+f*f);if(k<=pJS.particles.line_linked.distance){var e=pJS.particles.line_linked.color_rgb_line;
pJS.canvas.ctx.beginPath();pJS.canvas.ctx.strokeStyle="rgba("+e.r+","+e.g+","+e.b+","+(pJS.particles.line_linked.opacity-k/pJS.particles.line_linked.distance)+")";
...
j.vx+=i;j.vy+=h;}}};pJS.fn.vendors.interactivity.listeners=function(){pJS.canvas.el.onmousemove=function(f){if(pJS.retina){pJS.interactivity.mouse.pos_x=f.pageX*2;
pJS.interactivity.mouse.pos_y=f.pageY*2;}else{pJS.interactivity.mouse.pos_x=f.pageX;pJS.interactivity.mouse.pos_y=f.pageY;}pJS.interactivity.status="mousemove";
};pJS.canvas.el.onmouseleave=function(f){pJS.interactivity.mouse.pos_x=0;pJS.interactivity.mouse.pos_y=0;pJS.interactivity.status="mouseleave";};};pJS.fn.vendors.interactivity.grabParticles=function(j,i){var m=j.x-i.x,k=j.y-i.y,h=Math.sqrt(m*m+k*k);
var l=j.x-pJS.interactivity.mouse.pos_x,f=j.y-pJS.interactivity.mouse.pos_y,g=Math.sqrt(l*l+f*f);if(h<=pJS.particles.line_linked.distance&&g<=pJS.interactivity.mouse.distance&&pJS.interactivity.status=="mousemove"){var e=pJS.particles.line_linked.color_rgb_line;
